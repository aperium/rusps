validate_address_usps <- function(street, city, state, username) {
  ...
  scheme_host_path <- paste0("http://production.shippingapis.com/ShippingAPITest.dll?API=ZipCodeLookup&XML=<ZipCodeLookupRequest USERID=\"")
  ...
  # request in batches of five addresses
  for(j in 0:(groups-1))  {
    ...
    for(i in 0:4) {
      ...
      address_part  <- paste0("<Address1></Address1><Address2>",
                              street[(i + 1) + (j * 5)], "</Address2><City>",
                              city[(i + 1) + (j * 5)], "</City><State>",
                              state[(i + 1) + (j * 5)], "</State></Address>")
    ...
    address_list[index.errors] <- 
      structure(list(Address = structure(list(Address2 = NA,
                                              City = NA, State = NA, Zip5 = NA, Zip4 = NA,
                                              .attrs = structure("0", .Names = "ID")), 
                                         .Names = c("Address2","City", "State", "Zip5", "Zip4", ".attrs"))), 
                .Names = "Address")
  ...
  # request in individual addresses
